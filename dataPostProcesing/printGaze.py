...
def transformData(data, offset):
    proportion = [30.0,30.0]
    result =[]
    for r in data:
        result.append((int(((float(r[0] ) + offset[0])/ proportion[0]) * halfFramePepper[0] ) ,
                       int(((float(r[1]) + offset[1])/proportion[1]) * halfFramePepper[1])))
    return result


def print_gaze(gaze_file, offset, source_video, output_video, bacground):
    data = load_data_set(gaze_file)
    tranformedData = transformData(data, offset)
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    out = cv2.VideoWriter(output_video, fourcc, 9.0, frameSize)
    cap = cv2.VideoCapture(source_video)
    img = cv2.imread(bacground)
    i = 0
    couter = 11
    faces = None
    while (cap.isOpened()):
...
if __name__ == '__main__':
    ...
    tranformedData = transformData(data, GazeOfset)
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
